import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */import{D as I,S as E,L as q,a as F,I as M,b as C}from"./hero-data-BJ8VWDNA.js";import{E as O,S as N,R}from"./skill-categories-BKt7MNVS.js";const B="hero-state",L="hero-state-version",H=1,z=document.querySelector("#app");function P(){try{const t=localStorage.getItem(L);if(String(t)!==String(H))return localStorage.removeItem(B),localStorage.setItem(L,H),structuredClone(I);const s=localStorage.getItem(B);if(s)return{...structuredClone(I),...JSON.parse(s)}}catch{}return localStorage.setItem(L,H),structuredClone(I)}function v(){localStorage.setItem(B,JSON.stringify(e))}let e=P();function K(t){return t>.5?"#8b1a1a":t>.25?"#6a1515":"#4a0e0e"}function h(t){return 5+Math.floor(t/2)*5}function x(t){return 1+Math.floor(t/2)}function G(){const t=[{label:"Aktiv — Gegner-Debuffs",ids:[]},{label:"Aktiv — Selbst-Buffs",ids:[]},{label:"Aktiv — Schurke",ids:[]},{label:"Passive",ids:[]}];for(const s of Object.keys(F)){const a=E[s];a&&(a.type==="P"?t[3].ids.push(s):O.includes(s)?t[0].ids.push(s):N.includes(s)?t[1].ids.push(s):R.includes(s)?t[2].ids.push(s):a.desc.toLowerCase().includes("gegner")?t[0].ids.push(s):t[1].ids.push(s))}return t}const A={belt:"🦴",arm_protectors:"🛡",pelt:"🧊",small_bag:"👜",drinking_horn:"🥃",sword:"⚔"};function j(t){switch(t){case"belt":return"Außenkreis-Schaden reduziert um 1";case"arm_protectors":return"Mittelkreis-Schaden reduziert um 1";case"pelt":return"Innenkreis-Schaden reduziert um 1";case"small_bag":return"+1 Skill-Slot";case"drinking_horn":return`Heilen +${h(e.level)} einmal pro Kampf`;case"sword":return"2 aufeinanderfolgende Innentreffer = 1 Bonuswurf";default:return""}}function W(){return Object.entries(C).map(([t,s])=>{const a=e.unlockedItemIds.includes(t),i=a?"item-row":"item-row locked",u=A[t]||"?";let d;a?d="":d=`
        ...
        </div>`;const o=a?`<div class="item-effect">${j(t)}</div>`:"";return`
      <div class="${i}">
      ...
      </div>`)}return t.join("")}function V(){const t=e.maxHp>0?e.currentHp/e.maxHp:0,s=Math.round(t*100),a=Array.from({length:e.maxSkillSlots},(i,u)=>{const d=e.equippedSkillIds[u];if(!d)return'<div class="skill-slot empty-slot"><span class="empty-slot-text">— Leer —</span></div>';const o=E[d];if(!o)return"";const f=o.type==="A",b=f?"active":"passive",g=e.skillUsage[d]??o.maxUses,k=f?`<button class="skill-use-btn" data-skill-id="${d}" ${g<=0?"disabled":""}>Use<br><span class="btn-uses">${g}/${o.maxUses}</span></button>`:'<span class="skill-counter">&infin;</span>';return`
  ...
  </div>`;function Q(){const t=e.level+1,s=q.map(a=>`<button class="level-up-choice" data-choice="${a.id}">${a.label}</button>`).join("");return`
  ...
  <div class="skill-menu-overlay" id="skillmenu-overlay">
    <div class="skill-menu-panel">
      ...
      <h2 class="skill-menu-title">Skills</h2>`;for(const a of t)if(a.ids.length!==0){s+=`<div class="skill-group-header">${a.label}</div>`;for(const i of a.ids){const u=E[i],d=e.unlockedSkillIds.includes(i),o=e.equippedSkillIds.includes(i),f=e.equippedSkillIds.length<e.maxSkillSlots,b=d?"skill-row":"skill-row locked",g=u.type==="A"?"active":"passive";let k;d?o?k=`<button class="unequip-menu-btn" data-menu-unequip="${i}">Unequip</button>`:k=`<button class="equip-btn" data-equip="${i}" ${f?"":"disabled"}>Equip</button>`:k=`
          <div class="unlock-form">
  ...
  </div>`}function c(t){z.innerHTML=V()+Y()+Z+Q()+X()+ee()+te(t)+se(),document.querySelectorAll(".hp-btn").forEach(n=>{n.addEventListener("click",()=>{if(n.dataset.dir==="up"){if(e.currentHp>=e.maxHp||!confirm("Letzten Treffer rückgängig machen?"))return;e.currentHp++}n.dataset.dir==="down"&&e.currentHp>0&&e.currentHp--,v(),c()})}),document.querySelectorAll(".skill-use-btn").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.skillId,r=E[l];if(!r)return;const m=e.skillUsage[l]??r.maxUses;m>0&&(e.skillUsage[l]=m-1,v(),c())})});const s=document.getElementById("start-fight"),a=document.getElementById("fight-ended");s&&s.addEventListener("click",()=>{confirm("Kampf starten? Regeneration wird pausiert.")&&(e.inFight=!0,v(),c())}),a&&a.addEventListener("click",()=>{confirm("Kampf beenden? Skill-Einsätze werden wiederhergestellt und Regeneration wird fortgesetzt.")&&(e.skillUsage={},e.inFight=!1,e.drinkingHornUsed=!1,e.swordInnerStreak=0,e.swordBonusThrow=!1,p.nextTickAt=Date.now()+6e4,w(),v(),c())}),document.getElementById("heal-btn").addEventListener("click",()=>{if(e.currentHp>=e.maxHp||e.inFight)return;const n=h(e.level);confirm(`${n} HP heilen? Nur verwenden, wenn die erforderliche Aktion abgeschlossen wurde.`)&&(e.currentHp=Math.min(e.currentHp+n,e.maxHp),v(),c())}),document.querySelector(".reset-btn").addEventListener("click",()=>{confirm("WARNUNG: Dies löscht den GESAMTEN Fortschritt (Level, Skills, Items). Nur für Notfälle. Fortfahren?")&&confirm("Bist du absolut sicher? Dies kann nicht rückgängig gemacht werden.")&&(e=structuredClone(I),localStorage.removeItem(_),p={nextTickAt:Date.now()+6e4},v(),c())}),T("story-overlay","story-open","story-close"),T("info-overlay","info-open","info-close");const i=document.getElementById("levelup-overlay");document.getElementById("level-up-open").addEventListener("click",()=>{i.classList.add("visible")}),document.getElementById("levelup-cancel").addEventListener("click",()=>{i.classList.remove("visible")}),i.addEventListener("click",n=>{n.target===i&&i.classList.remove("visible")}),document.querySelectorAll(".level-up-choice").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.choice,r=q.find(D=>D.id===l);if(!r)return;const m=e.level+1,y=m%5===0;if(!confirm(`Level Up auf ${m}?
${r.label}`))return;r.apply(e),e.level=m,e.levelUpHistory.push(l);const S=[r.label];y&&(e.maxSkillSlots+=1,e.actions<3?(e.actions+=1,S.push("+1 Aktion, +1 Skill-Slot")):S.push("+1 Skill-Slot")),m%2===0&&(S.push(`Heilen steigt auf +${h(m)}`),S.push(`Regen steigt auf ${x(m)} HP/min`)),e.unlockedItemIds.includes("drinking_horn")&&m%2===0&&S.push(`Drinking Horn Heilung steigt auf +${h(m)}`),v(),i.classList.remove("visible"),c(S),document.getElementById("levelup-summary-overlay")?.classList.add("visible")})});const u=document.getElementById("skillmenu-overlay");document.getElementById("skill-menu-open").addEventListener("click",()=>{u.classList.add("visible")}),document.getElementById("skillmenu-close").addEventListener("click",()=>{u.classList.remove("visible")}),u.addEventListener("click",n=>{n.target===u&&u.classList.remove("visible")});const d=document.getElementById("itemmenu-overlay");document.getElementById("item-menu-open").addEventListener("click",()=>{d.classList.add("visible")}),document.getElementById("itemmenu-close").addEventListener("click",()=>{d.classList.remove("visible")}),d.addEventListener("click",n=>{n.target===d&&d.classList.remove("visible")});const o=document.getElementById("defeat-overlay");o&&(document.getElementById("defeat-close").addEventListener("click",()=>{o.classList.remove("visible")}),o.addEventListener("click",n=>{n.target===o&&o.classList.remove("visible")}));const f=document.getElementById("levelup-summary-overlay");f&&(document.getElementById("levelup-summary-close").addEventListener("click",()=>{f.classList.remove("visible")}),f.addEventListener("click",n=>{n.target===f&&f.classList.remove("visible")})),document.querySelectorAll(".unlock-btn").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.unlock,r=document.querySelector(`.unlock-input[data-skill-id="${l}"]`),m=r.value.trim(),y=F[l];if(m===y){if(e.unlockedSkillIds.includes(l))return;e.unlockedSkillIds.push(l),v(),c(),document.getElementById("skillmenu-overlay").classList.add("visible")}else r.classList.add("error"),r.value="",setTimeout(()=>r.classList.remove("error"),400)})}),document.querySelectorAll(".equip-btn").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.equip;if(e.equippedSkillIds.length>=e.maxSkillSlots||e.equippedSkillIds.includes(l))return;e.equippedSkillIds.push(l);const r=E[l];r&&!(l in e.skillUsage)&&(e.skillUsage[l]=r.maxUses),v(),c(),document.getElementById("skillmenu-overlay").classList.add("visible")})}),document.querySelectorAll(".unequip-menu-btn").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.menuUnequip;e.equippedSkillIds=e.equippedSkillIds.filter(r=>r!==l),v(),c(),document.getElementById("skillmenu-overlay").classList.add("visible")})}),document.querySelectorAll(".item-unlock-btn").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.itemUnlock,r=document.querySelector(`.unlock-input[data-item-id="${l}"]`),m=r.value.trim(),y=M[l];if(m===y){if(e.unlockedItemIds.includes(l))return;e.unlockedItemIds.push(l),l==="small_bag"&&(e.maxSkillSlots+=1),v(),c(),document.getElementById("itemmenu-overlay").classList.add("visible")}else r.classList.add("error"),r.value="",setTimeout(()=>r.classList.remove("error"),400)})});const b=document.getElementById("drinking-horn-use");b&&b.addEventListener("click",()=>{if(e.drinkingHornUsed||e.currentHp>=e.maxHp)return;const n=h(e.level);e.currentHp=Math.min(e.currentHp+n,e.maxHp),e.drinkingHornUsed=!0,v(),c()});const g=document.getElementById("sword-inner-hit");g&&g.addEventListener("click",()=>{e.swordInnerStreak+=1,e.swordInnerStreak>=2&&(e.swordInnerStreak=0,e.swordBonusThrow=!0),v(),c()});const k=document.getElementById("sword-miss");k&&k.addEventListener("click",()=>{e.swordInnerStreak=0,v(),c()});const U=document.getElementById("sword-use-bonus");U&&U.addEventListener("click",()=>{e.swordBonusThrow=!1,v(),c()}),typeof p<"u"&&$()}function T(t,s,a){const i=document.getElementById(t);document.getElementById(s).addEventListener("click",()=>{i.classList.add("visible")}),document.getElementById(a).addEventListener("click",()=>{i.classList.remove("visible")}),i.addEventListener("click",u=>{u.target===i&&i.classList.remove("visible")})}const _="hero-regen";function ne(){try{const t=localStorage.getItem(_);if(t)return JSON.parse(t)}catch{}return{nextTickAt:Date.now()+6e4}}function w(){localStorage.setItem(_,JSON.stringify(p))}let p=ne();p.secondsLeft!=null&&p.nextTickAt==null&&(p={nextTickAt:Date.now()+p.secondsLeft*1e3},w());function ie(){if(e.inFight||e.currentHp<=0){$();return}const t=Date.now();if(t>=p.nextTickAt){if(p.nextTickAt=t+6e4,e.currentHp<e.maxHp){const s=x(e.level);e.currentHp=Math.min(e.currentHp+s,e.maxHp),v(),w(),c();return}w()}$()}function $(){const t=document.getElementById("regen-timer");if(!t)return;const s=Math.max(0,Math.ceil((p.nextTickAt-Date.now())/1e3));t.textContent=`(${s}s)`}setInterval(ie,1e3);c();$();
