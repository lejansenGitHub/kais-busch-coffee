import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */const v=document.createElement("style");v.textContent=`
  .back-link {
  ...
  .slide-nav {
    ...
    padding: 12px 20px;
    background: linear-gradient(180deg, rgba(10, 8, 4, 0.0) 0%, rgba(10, 8, 4, 0.95) 20%);
    z-index: 100;
  }
  .slide-nav-btn {
...
`;document.head.appendChild(v);const t=10;let i=0;const r=[`<div class="guide-section">
    <div class="guide-text">Willkommen bei <strong>Lord Pattens Abenteuer</strong> — einem Outdoor-Wurfspiel, bei dem du mit Sandsäcken auf eine Zielscheibe wirfst, Quests erfüllst und Gegner besiegst.</div>
    <div class="guide-text">Diese Anleitung erklärt dir alles, was du wissen musst — Slide für Slide. Wische nach links oder tippe auf <strong>Weiter</strong>, um zu beginnen.</div>
    ...
    <div class="section-title">Die Zielscheibe</div>
    ...
    <div class="section-title">Sandsack-Qualitäten</div>
    ...
    <div class="section-title">Spielablauf</div>
    ...
    <div class="section-title">Schaden & HP</div>
    ...
    <div class="section-title">Belohnungen & Level Up</div>
    ...
    <div class="section-title">Niederlagen & Wiederholung</div>
  ...
  </div>`],a=document.getElementById("app");a.innerHTML=`
  ...
  <div class="character-card">
    ...
    <div class="slide-container">
      ${r.map((e,s)=>`<div class="slide${s===0?" active":""}" data-slide="${s}">${e}</div>`).join("")}
    </div>
  </div>
  <div class="slide-nav">
    <button class="slide-nav-btn" id="prevBtn">← Zurück</button>
    <div>
      <div class="slide-counter" id="slideCounter">1 / ${t}</div>
      <div class="slide-dots" id="slideDots">
        ${r.map((e,s)=>`<div class="slide-dot${s===0?" active":""}"></div>`).join("")}
      </div>
    </div>
    <button class="slide-nav-btn" id="nextBtn">Weiter →</button>
  </div>
`;const l=a.querySelectorAll(".slide"),g=document.getElementById("prevBtn"),d=document.getElementById("nextBtn"),x=document.getElementById("slideCounter"),c=document.getElementById("slideDots").children;function n(e){e<0||e>=t||(l[i].classList.remove("active"),c[i].classList.remove("active"),i=e,l[i].classList.add("active"),c[i].classList.add("active"),x.textContent=`${i+1} / ${t}`,g.classList.toggle("hidden",i===0),i===t-1?d.textContent="Fertig ✓":d.textContent="Weiter →",a.scrollIntoView({behavior:"instant",block:"start"}))}g.addEventListener("click",()=>n(i-1));d.addEventListener("click",()=>{i===t-1?window.location.href="../":n(i+1)});n(0);document.addEventListener("keydown",e=>{e.key==="ArrowRight"||e.key==="ArrowDown"?(e.preventDefault(),i<t-1&&n(i+1)):(e.key==="ArrowLeft"||e.key==="ArrowUp")&&(e.preventDefault(),n(i-1))});let f=0,p=0,o=0,u=0;a.addEventListener("touchstart",e=>{f=e.changedTouches[0].screenX,p=e.changedTouches[0].screenY},{passive:!0});a.addEventListener("touchend",e=>{o=e.changedTouches[0].screenX,u=e.changedTouches[0].screenY;const s=o-f,h=u-p;Math.abs(s)>50&&Math.abs(s)>Math.abs(h)*1.5&&(s<0&&i<t-1?n(i+1):s>0&&i>0&&n(i-1))},{passive:!0});
