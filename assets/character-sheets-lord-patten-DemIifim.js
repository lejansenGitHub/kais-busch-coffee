import"./modulepreload-polyfill-B5Qt9EMX.js";import{S as f}from"./skills-C6ifZiiC.js";import{D as h,L as E,S as I}from"./hero-data-Cf1XjC3Q.js";const L="hero-state",H=document.querySelector("#app");function q(){try{const s=localStorage.getItem(L);if(s)return JSON.parse(s)}catch{}return structuredClone(h)}function r(){localStorage.setItem(L,JSON.stringify(e))}let e=q();function w(s){return s>.5?"#8b1a1a":s>.25?"#6a1515":"#4a0e0e"}function k(s){return 5+Math.floor(s/2)*5}function b(s){return 1+Math.floor(s/2)}function A(){const s=[{label:"Active — Enemy Debuffs",ids:[]},{label:"Active — Self Buffs",ids:[]},{label:"Passive",ids:[]}];for(const[a,o]of Object.entries(f))o.type==="A"?["advance","double_attack","fine_sack","snipers_gambit","war_cry"].includes(a)?s[1].ids.push(a):s[0].ids.push(a):s[2].ids.push(a);return s}function x(){const s=e.maxHp>0?e.currentHp/e.maxHp:0,a=Math.round(s*100),o=Array.from({length:e.maxSkillSlots},(n,t)=>{const l=e.equippedSkillIds[t];if(!l)return'<div class="skill-slot empty-slot"><span class="empty-slot-text">— Empty —</span></div>';const i=f[l];if(!i)return"";const c=i.type==="A",u=c?"active":"passive",p=e.skillUsage[l]??i.maxUses,v=c?`<button class="skill-use-btn" data-skill-id="${l}" ${p<=0?"disabled":""}>Use<br><span class="btn-uses">${p}/${i.maxUses}</span></button>`:'<span class="skill-counter">&infin;</span>';return`
    ...
    </div>`}function B(){const s=h.story;return`
  ...
  </div>`}function _(){const s=A();let a=`
