import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */import{D as E,S as $,L as _,d as S,a as q,I as z,b as C}from"./hero-data-B8v1JvdX.js";const F="hero-state",O=document.querySelector("#app");function R(){try{const s=localStorage.getItem(F);if(s){const t=JSON.parse(s);return t.unlockedItemIds||(t.unlockedItemIds=[]),t.drinkingHornUsed===void 0&&(t.drinkingHornUsed=!1),t.swordInnerStreak===void 0&&(t.swordInnerStreak=0),t.swordBonusThrow===void 0&&(t.swordBonusThrow=!1),t}}catch{}return structuredClone(E)}function m(){localStorage.setItem(F,JSON.stringify(e))}let e=R();function N(s){return s>.5?"#8b1a1a":s>.25?"#6a1515":"#4a0e0e"}function h(s){return 5+Math.floor(s/2)*5}function B(s){return 1+Math.floor(s/2)}function D(){const s=[{label:"Aktiv — Gegner",ids:[]},{label:"Aktiv — Selbst",ids:[]},{label:"Passive",ids:[]}];for(const t of Object.keys(q)){const d=$[t];d&&(d.type==="A"?["advance","double_attack","fine_sack","snipers_gambit","war_cry"].includes(t)?s[1].ids.push(t):s[0].ids.push(t):s[2].ids.push(t))}return s}const H={belt:"🦴",arm_protectors:"🛡",pelt:"🧊",small_bag:"👜",drinking_horn:"🥃",sword:"⚔"};function P(s){const t=S(e.level);switch(s){case"belt":return`Außenkreis-Schaden reduziert um ${t}`;case"arm_protectors":return`Mittelkreis-Schaden reduziert um ${t}`;case"pelt":return`Innenkreis-Schaden reduziert um ${t}`;case"small_bag":return"+1 Skill-Slot";case"drinking_horn":return`Heilen +${h(e.level)} einmal pro Kampf`;case"sword":return"2 aufeinanderfolgende Innentreffer = 1 Bonuswurf";default:return""}}function K(){return Object.entries(C).map(([s,t])=>{const d=e.unlockedItemIds.includes(s),i=d?"item-row":"item-row locked",u=H[s]||"?";let o;d?o="":o=`
      ...
      </div>`)}return s.join("")}function G(){const s=e.maxHp>0?e.currentHp/e.maxHp:0,t=Math.round(s*100),d=Array.from({length:e.maxSkillSlots},(i,u)=>{const o=e.equippedSkillIds[u];if(!o)return'<div class="skill-slot empty-slot"><span class="empty-slot-text">— Leer —</span></div>';const a=$[o];if(!a)return"";const f=a.type==="A",b=f?"active":"passive",k=e.skillUsage[o]??a.maxUses,p=f?`<button class="skill-use-btn" data-skill-id="${o}" ${k<=0?"disabled":""}>Use<br><span class="btn-uses">${k}/${a.maxUses}</span></button>`:'<span class="skill-counter">&infin;</span>';return`
  ...
  <div class="skill-menu-overlay" id="skillmenu-overlay">
    <div class="skill-menu-panel">
      ...
      <h2 class="skill-menu-title">Skills</h2>`;for(const d of s)if(d.ids.length!==0){t+=`<div class="skill-group-header">${d.label}</div>`;for(const i of d.ids){const u=$[i],o=e.unlockedSkillIds.includes(i),a=e.equippedSkillIds.includes(i),f=e.equippedSkillIds.length<e.maxSkillSlots,b=o?"skill-row":"skill-row locked",k=u.type==="A"?"active":"passive";let p;o?a?p=`<button class="unequip-menu-btn" data-menu-unequip="${i}">Unequip</button>`:p=`<button class="equip-btn" data-equip="${i}" ${f?"":"disabled"}>Equip</button>`:p=`
          <div class="unlock-form">
