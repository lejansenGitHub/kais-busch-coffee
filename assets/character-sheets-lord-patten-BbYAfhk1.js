import"./modulepreload-polyfill-B5Qt9EMX.js";import{S as m}from"./skills-Bw_GagAK.js";const h={id:"lord-patten",name:"Lord Patten",level:1,maxHp:20,currentHp:20,actions:1,damage:{outer:1,middle:3,inner:5},maxSkillSlots:1,unlockedSkillIds:[],equippedSkillIds:[],skillUsage:{},inFight:!1,levelUpHistory:[],items:[],story:{desc:"A noble warrior of unyielding resolve. Once exiled from the courts of the High Keep, he wanders the land seeking worthy foes — and a good cup of coffee.",onHit:`"Is that all you've got?"`,onStrike:'"For honor."',onDefeat:`"This isn't over. Not by a long shot."`}},S={blind_fury:"4812",repulsion:"7391",pushback:"2650",iron_ward:"8174",off_balance:"5923",heavy_hands:"3046",freeze:"6287",crude_sack:"1435",tremor:"9768",shake:"5102",vertigo:"8341",weighted_sack:"2079",advance:"6514",double_attack:"3897",fine_sack:"7243",snipers_gambit:"4086",war_cry:"1952",sturdy_arm:"8625",worn_grip:"3471",titans_grip:"7908",last_stand:"5236",fortress:"9014",intimidate:"2763",inspire:"6180"},b=[{id:"hp10",label:"+10 HP",apply(s){s.maxHp+=10,s.currentHp+=10}},{id:"inner5",label:"+5 Inner damage",apply(s){s.damage.inner+=5}},{id:"inner2_middle2",label:"+2 Inner, +2 Middle damage",apply(s){s.damage.inner+=2,s.damage.middle+=2}},{id:"all1",label:"+1 All zones damage",apply(s){s.damage.outer+=1,s.damage.middle+=1,s.damage.inner+=1}}],y="hero-state",E=document.querySelector("#app");function $(){try{const s=localStorage.getItem(y);if(s)return JSON.parse(s)}catch{}return structuredClone(h)}function r(){localStorage.setItem(y,JSON.stringify(e))}let e=$();function L(s){return s>.5?"#8b1a1a":s>.25?"#6a1515":"#4a0e0e"}function f(s){return 5+Math.floor(s/2)*5}function g(s){return 1+Math.floor(s/2)}function H(){const s=[{label:"Active — Enemy Debuffs",ids:[]},{label:"Active — Self Buffs",ids:[]},{label:"Passive",ids:[]}];for(const[a,o]of Object.entries(m))o.type==="A"?["advance","double_attack","fine_sack","snipers_gambit","war_cry"].includes(a)?s[1].ids.push(a):s[0].ids.push(a):s[2].ids.push(a);return s}function I(){const s=e.maxHp>0?e.currentHp/e.maxHp:0,a=Math.round(s*100),o=Array.from({length:e.maxSkillSlots},(n,t)=>{const l=e.equippedSkillIds[t];if(!l)return'<div class="skill-slot empty-slot"><span class="empty-slot-text">— Empty —</span></div>';const i=m[l];if(!i)return"";const c=i.type==="A",u=c?"active":"passive",p=e.skillUsage[l]??i.maxUses,v=c?`<button class="skill-use-btn" data-skill-id="${l}" ${p<=0?"disabled":""}>Use<br><span class="btn-uses">${p}/${i.maxUses}</span></button>`:'<span class="skill-counter">&infin;</span>';return`
    ...
    <div class="character-card">
      ...
      <div class="hp-section">
        <div class="hp-bar-wrapper">
          ...
          <div class="hp-bar-track">
            <div class="hp-bar-fill" style="width:${a}%;background:${L(s)}"></div>
      ...
      <div class="actions-line">
        <span class="actions-label">Regen${e.inFight?" (paused)":""}</span>
        <span class="actions-value${e.inFight?" regen-paused":""}">${g(e.level)} HP/min</span>
      </div>
    ...
    </div>`}function w(){const s=h.story;return`
  ...
  </div>`;function _(){const s=e.level+1,a=s%5===0,o=s%2===0,n=[];a&&n.push("+1 Action, +1 Skill Slot"),o&&n.push(`Heal increases to +${f(s)}`,`Regen increases to ${g(s)} HP/min`);const t=n.length>0?`<div class="level-up-bonus">${n.join("<br>")}</div>`:"",l=b.map(i=>`<button class="level-up-choice" data-choice="${i.id}">${i.label}</button>`).join("");return`
  ...
  </div>`}function x(){const s=H();let a=`
  <div class="skill-menu-overlay" id="skillmenu-overlay">
    <div class="skill-menu-panel">
      ...
      <h2 class="skill-menu-title">Skills</h2>`;for(const o of s)if(o.ids.length!==0){a+=`<div class="skill-group-header">${o.label}</div>`;for(const n of o.ids){const t=m[n],l=e.unlockedSkillIds.includes(n),i=e.equippedSkillIds.includes(n),c=e.equippedSkillIds.length<e.maxSkillSlots,u=l?"skill-row":"skill-row locked",p=t.type==="A"?"active":"passive";let v;l?i?v='<span class="equipped-badge">Equipped</span>':v=`<button class="equip-btn" data-equip="${n}" ${c?"":"disabled"}>Equip</button>`:v=`
        ...
        </div>`}}return a+="</div></div>",a}function d(){E.innerHTML=I()+w()+q+_()+x(),document.querySelectorAll(".hp-btn").forEach(t=>{t.addEventListener("click",()=>{if(t.dataset.dir==="up"){if(e.currentHp>=e.maxHp||!confirm("Undo last hit?"))return;e.currentHp++}t.dataset.dir==="down"&&e.currentHp>0&&e.currentHp--,r(),d()})}),document.querySelectorAll(".skill-use-btn").forEach(t=>{t.addEventListener("click",()=>{const l=t.dataset.skillId,i=m[l];if(!i)return;const c=e.skillUsage[l]??i.maxUses;c>0&&(e.skillUsage[l]=c-1,r(),d())})}),document.querySelectorAll(".unequip-btn").forEach(t=>{t.addEventListener("click",()=>{const l=t.dataset.unequip;e.equippedSkillIds=e.equippedSkillIds.filter(i=>i!==l),r(),d()})});const s=document.getElementById("start-fight"),a=document.getElementById("fight-ended");s&&s.addEventListener("click",()=>{confirm("Start a fight? Regeneration will be paused.")&&(e.inFight=!0,r(),d())}),a&&a.addEventListener("click",()=>{confirm("End the current fight? Skill uses will be restored and regeneration will resume.")&&(e.skillUsage={},e.inFight=!1,r(),d())}),document.getElementById("heal-btn").addEventListener("click",()=>{if(e.currentHp>=e.maxHp||e.inFight)return;const t=f(e.level);confirm(`Heal ${t} HP? Only use this if you completed the required action.`)&&(e.currentHp=Math.min(e.currentHp+t,e.maxHp),r(),d())}),document.querySelector(".reset-btn").addEventListener("click",()=>{confirm("WARNING: This will erase ALL progress (level, skills, items). This is for emergencies only. Continue?")&&confirm("Are you absolutely sure? This cannot be undone.")&&(e=structuredClone(h),r(),d())}),k("story-overlay","story-open","story-close"),k("info-overlay","info-open","info-close");const o=document.getElementById("levelup-overlay");document.getElementById("level-up-open").addEventListener("click",()=>{o.classList.add("visible")}),document.getElementById("levelup-cancel").addEventListener("click",()=>{o.classList.remove("visible")}),o.addEventListener("click",t=>{t.target===o&&o.classList.remove("visible")}),document.querySelectorAll(".level-up-choice").forEach(t=>{t.addEventListener("click",()=>{const l=t.dataset.choice,i=b.find(v=>v.id===l);if(!i)return;const c=e.level+1,u=c%5===0,p=u?`
+1 Action, +1 Skill Slot`:"";confirm(`Level up to ${c}?
${i.label}${p}`)&&(i.apply(e),e.level=c,e.levelUpHistory.push(l),u&&(e.actions+=1,e.maxSkillSlots+=1),r(),o.classList.remove("visible"),d())})});const n=document.getElementById("skillmenu-overlay");document.getElementById("skill-menu-open").addEventListener("click",()=>{n.classList.add("visible")}),document.getElementById("skillmenu-close").addEventListener("click",()=>{n.classList.remove("visible")}),n.addEventListener("click",t=>{t.target===n&&n.classList.remove("visible")}),document.querySelectorAll(".unlock-btn").forEach(t=>{t.addEventListener("click",()=>{const l=t.dataset.unlock,i=document.querySelector(`.unlock-input[data-skill-id="${l}"]`),c=i.value.trim(),u=S[l];c===u?(e.unlockedSkillIds.push(l),r(),d(),document.getElementById("skillmenu-overlay").classList.add("visible")):(i.classList.add("error"),i.value="",setTimeout(()=>i.classList.remove("error"),400))})}),document.querySelectorAll(".equip-btn").forEach(t=>{t.addEventListener("click",()=>{const l=t.dataset.equip;if(e.equippedSkillIds.length>=e.maxSkillSlots||e.equippedSkillIds.includes(l))return;e.equippedSkillIds.push(l);const i=m[l];i&&!(l in e.skillUsage)&&(e.skillUsage[l]=i.maxUses),r(),d(),document.getElementById("skillmenu-overlay").classList.add("visible")})})}function k(s,a,o){const n=document.getElementById(s);document.getElementById(a).addEventListener("click",()=>{n.classList.add("visible")}),document.getElementById(o).addEventListener("click",()=>{n.classList.remove("visible")}),n.addEventListener("click",t=>{t.target===n&&n.classList.remove("visible")})}setInterval(()=>{if(e.inFight||e.currentHp>=e.maxHp)return;const s=g(e.level);e.currentHp=Math.min(e.currentHp+s,e.maxHp),r(),d()},6e4);d();
