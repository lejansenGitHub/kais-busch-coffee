import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */import{S as $}from"./skills-CpN84Atv.js";import{D as E,L as _,d as S,S as M,I as z,a as C}from"./hero-data-Dk_-gBUj.js";const q="hero-state",O=document.querySelector("#app");function R(){try{const s=localStorage.getItem(q);if(s){const t=JSON.parse(s);return t.unlockedItemIds||(t.unlockedItemIds=[]),t.drinkingHornUsed===void 0&&(t.drinkingHornUsed=!1),t.swordInnerStreak===void 0&&(t.swordInnerStreak=0),t.swordBonusThrow===void 0&&(t.swordBonusThrow=!1),t}}catch{}return structuredClone(E)}function m(){localStorage.setItem(q,JSON.stringify(e))}let e=R();function N(s){return s>.5?"#8b1a1a":s>.25?"#6a1515":"#4a0e0e"}function g(s){return 5+Math.floor(s/2)*5}function B(s){return 1+Math.floor(s/2)}function D(){const s=[{label:"Aktiv — Gegner",ids:[]},{label:"Aktiv — Selbst",ids:[]},{label:"Passive",ids:[]}];for(const[t,a]of Object.entries($))a.type==="A"?["advance","double_attack","fine_sack","snipers_gambit","war_cry"].includes(t)?s[1].ids.push(t):s[0].ids.push(t):s[2].ids.push(t);return s}const H={belt:"🦴",arm_protectors:"🛡",pelt:"🧊",small_bag:"👜",drinking_horn:"🥃",sword:"⚔"};function P(s){const t=S(e.level);switch(s){case"belt":return`Außenkreis-Schaden reduziert um ${t}`;case"arm_protectors":return`Mittelkreis-Schaden reduziert um ${t}`;case"pelt":return`Innenkreis-Schaden reduziert um ${t}`;case"small_bag":return"+1 Skill-Slot";case"drinking_horn":return`Heilen +${g(e.level)} einmal pro Kampf`;case"sword":return"2 aufeinanderfolgende Innentreffer = 1 Bonuswurf";default:return""}}function K(){return Object.entries(C).map(([s,t])=>{const a=e.unlockedItemIds.includes(s),i=a?"item-row":"item-row locked",u=H[s]||"?";let o;a?o="":o=`
        ...
        </div>`;const d=a?`<div class="item-effect">${P(s)}</div>`:"";return`
      <div class="${i}">
      ...
      </div>`)}return s.join("")}function G(){const s=e.maxHp>0?e.currentHp/e.maxHp:0,t=Math.round(s*100),a=Array.from({length:e.maxSkillSlots},(i,u)=>{const o=e.equippedSkillIds[u];if(!o)return'<div class="skill-slot empty-slot"><span class="empty-slot-text">— Leer —</span></div>';const d=$[o];if(!d)return"";const f=d.type==="A",h=f?"active":"passive",k=e.skillUsage[o]??d.maxUses,p=f?`<button class="skill-use-btn" data-skill-id="${o}" ${k<=0?"disabled":""}>Use<br><span class="btn-uses">${k}/${d.maxUses}</span></button>`:'<span class="skill-counter">&infin;</span>';return`
  ...
  </div>`;function J(){const s=e.level+1,t=_.map(a=>`<button class="level-up-choice" data-choice="${a.id}">${a.label}</button>`).join("");return`
  ...
  <div class="skill-menu-overlay" id="skillmenu-overlay">
    <div class="skill-menu-panel">
      ...
      <h2 class="skill-menu-title">Skills</h2>`;for(const a of s)if(a.ids.length!==0){t+=`<div class="skill-group-header">${a.label}</div>`;for(const i of a.ids){const u=$[i],o=e.unlockedSkillIds.includes(i),d=e.equippedSkillIds.includes(i),f=e.equippedSkillIds.length<e.maxSkillSlots,h=o?"skill-row":"skill-row locked",k=u.type==="A"?"active":"passive";let p;o?d?p=`<button class="unequip-menu-btn" data-menu-unequip="${i}">Unequip</button>`:p=`<button class="equip-btn" data-equip="${i}" ${f?"":"disabled"}>Equip</button>`:p=`
          <div class="unlock-form">
